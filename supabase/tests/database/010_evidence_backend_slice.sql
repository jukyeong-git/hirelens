...
select throws_ok($$ select public.claim_evidence_processing_run('70000000-0000-0000-0000-000000000801') $$, '42501', 'worker service role required', 'browser role cannot claim evidence work');

set local role service_role;
select set_config('request.jwt.claim.role', 'service_role', true);
select lives_ok($$ select public.claim_evidence_processing_run('70000000-0000-0000-0000-000000000801') $$, 'worker claims evidence run');
...
select lives_ok($$
  select public.complete_resume_extraction_for_evidence('70000000-0000-0000-0000-000000000801', jsonb_build_array(jsonb_build_object('page_number', 1, 'raw_text', 'Built reliable backend services.', 'normalized_text', 'Built reliable backend services.', 'raw_text_sha256', repeat('b', 64), 'normalized_text_sha256', repeat('c', 64))))
...
select ok(jsonb_array_length(public.load_evidence_analysis_context('70000000-0000-0000-0000-000000000801') -> 'criteria') > 0, 'context loads human-approved criteria');
select lives_ok($$ select public.mark_evidence_validating('70000000-0000-0000-0000-000000000801', 'evidence-extraction-prompt-v1', 'evidence-extraction-schema-v1', 'mock-model', 'resp_mock', 100, 20, 120, 300, 25) $$, 'usage and contract metadata enter validation');
select throws_ok($$
...
select lives_ok($$ select public.persist_validated_evidence('70000000-0000-0000-0000-000000000801', '[]'::jsonb) $$, 'duplicate completion is idempotent');
...
set local role postgres;
update public.processing_runs set status = 'ANALYZING', completed_at = null, attempt_count = 1 where id = '70000000-0000-0000-0000-000000000801';
set local role service_role;
select set_config('request.jwt.claim.role', 'service_role', true);
select lives_ok($$ select public.record_evidence_processing_failure('70000000-0000-0000-0000-000000000801', 'AI_RATE_LIMIT', true, false, 'AI_RATE_LIMIT') $$, 'first transient failure schedules one retry');
...
select lives_ok($$ select public.claim_evidence_processing_run('70000000-0000-0000-0000-000000000801') $$, 'single retry is claimed');
select lives_ok($$ select public.record_evidence_processing_failure('70000000-0000-0000-0000-000000000801', 'AI_RATE_LIMIT', true, false, 'AI_RATE_LIMIT') $$, 'second failure exhausts attempts');
