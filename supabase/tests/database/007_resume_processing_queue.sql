...
select throws_ok($$ select public.claim_evidence_processing_run((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700')) $$, '42501', 'worker service role required', 'Browser roles cannot claim processing');
...
select lives_ok($$ select public.claim_evidence_processing_run((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700')) $$, 'Service role claims the queued run');
...
select lives_ok($$ select public.record_evidence_processing_failure((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700'), 'STORAGE_UNAVAILABLE', true, false, 'STORAGE_UNAVAILABLE') $$, 'First transient failure creates bounded retry');
...
select lives_ok($$ select public.claim_evidence_processing_run((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700')) $$, 'Service role claims the retry');
select lives_ok($$ select public.record_evidence_processing_failure((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700'), 'STORAGE_UNAVAILABLE', true, false, 'STORAGE_UNAVAILABLE') $$, 'Second transient failure is durable');
