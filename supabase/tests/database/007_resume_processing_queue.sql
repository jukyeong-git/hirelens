...
update public.jobs set status = 'READY_FOR_INTAKE' where id = '10000000-0000-0000-0000-000000000001' and status = 'DRAFT';
set local role authenticated;
select set_config('request.jwt.claim.sub', '00000000-0000-0000-0000-000000000001', true);

select lives_ok($$
  select public.create_resume_upload_reservation('10000000-0000-0000-0000-000000000001','40000000-0000-0000-0000-000000000700','50000000-0000-0000-0000-000000000700','60000000-0000-0000-0000-000000000700','10000000-0000-0000-0000-000000000001/50000000-0000-0000-0000-000000000700/60000000-0000-0000-0000-000000000700.pdf','resume.pdf','application/pdf',1024,repeat('a',64))
...
select throws_ok($$ select public.claim_evidence_processing_run((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700'), 300) $$, '42501', 'permission denied for function claim_evidence_processing_run', 'Browser roles cannot claim processing');
...
select ok(not has_function_privilege('service_role', 'public.create_human_review(uuid,uuid,public.human_decision,text,text,public.review_confidence,text)', 'EXECUTE'), 'Worker service role cannot invoke human decision RPC');

set local role service_role;
select set_config('request.jwt.claim.role', 'service_role', true);
select lives_ok($$ select public.claim_evidence_processing_run((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700'), 300) $$, 'Service role claims the queued run');
...
select lives_ok($$ select public.record_evidence_processing_failure((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700'), 'STORAGE_UNAVAILABLE', true, false, 'STORAGE_UNAVAILABLE', (select lease_token from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700')) $$, 'First transient failure creates bounded retry');
select is((select status::text from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700'), 'RETRY_PENDING', 'Retry is an explicit state');
select lives_ok($$ select public.claim_evidence_processing_run((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700'), 300) $$, 'Service role claims the retry');
select lives_ok($$ select public.record_evidence_processing_failure((select id from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700'), 'STORAGE_UNAVAILABLE', true, false, 'STORAGE_UNAVAILABLE', (select lease_token from public.processing_runs where resume_file_id = '60000000-0000-0000-0000-000000000700')) $$, 'Second transient failure is durable');
