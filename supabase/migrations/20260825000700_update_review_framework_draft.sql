-- Allow an authorized human to revise a saved Review Framework draft.
-- Approved versions remain immutable; they must use create_scorecard_revision.
-- Rollback: revoke/drop this RPC in a forward migration. Existing audit events
-- and already-updated draft contents must remain append-only.
...
begin
  ...
  if normalized_reason = '' or length(normalized_reason) > 1000 then
    raise exception 'valid update reason is required' using errcode = '22023';
  end if;
  ...
  insert into public.audit_events (
    ...
    normalized_reason, 'scorecard_draft_update', 'SUCCESS', target_scorecard_version_id::text
  );
end;
$$;
